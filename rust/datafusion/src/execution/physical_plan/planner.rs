    ...
    fn create_initial_plan(
        ...
        match logical_plan {
            LogicalPlan::TableScan {
                ...
                match ctx_state.get_engine_name(table_schema) {
                    Some(engine_name) => {
                        match ctx_state.get_table_provider(engine_name) {
                            Some(provider) => provider.scan(schema_name, table_name, table_schema, projection, batch_size),
